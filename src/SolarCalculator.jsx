...
export default function SolarCalculator() {
    ...
    sugTimer.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&limit=5&countrycodes=us&addressdetails=0&q=${encodeURIComponent(v)}`
        ).then((x) => x.json());
  ...
  const pickSuggestion = (sug) => {
    ...
    setSuggestions([]);
    fetchSolarFor(sug.lat, sug.lon, sug.label);
  };
  ...
  const lookupAddress = async () => {
    ...
    try {
      ...
      await fetchSolarFor(parseFloat(lat), parseFloat(lon), display_name.split(",").slice(0, 3).join(","));
    } catch {
      setGeoState("error");
  ...
  const fetchSolarFor = async (la, lo, shortName) => {
    setGeoState("loading");
    ...
    try {
      ...
      // rough utility territory guess from coordinates
      let guessed = "";
      if (la > 32.4 && la < 33.6 && lo > -118 && lo < -115.8) { pickUtilityKeepSun("sdge", psh); guessed = " · looks like SDG&E territory"; }
      else if (la > 35.6 && la < 42.2 && lo > -125 && lo < -118.8) { pickUtilityKeepSun("pge", psh); guessed = " · looks like PG&E territory"; }
...
Use null for anything not found. Numbers only, no units or symbols.`
      ...
      const applied = [];
      if (parsed.utility === "SDG&E") { pickUtility("sdge"); applied.push("utility: SDG&E"); }
      else if (parsed.utility === "PG&E") { pickUtility("pge"); applied.push("utility: PG&E"); }
      if (parsed.avg_rate && parsed.avg_rate > 0.05 && parsed.avg_rate < 1) { setRate(parsed.avg_rate); applied.push(`rate $${parsed.avg_rate.toFixed(2)}/kWh`); }
    ...
    <div className="app-root" style={{ minHeight: "100vh", background: C.night, color: C.text, fontFamily: "'Space Grotesk', 'Segoe UI', sans-serif" }}>
        ...
        <div style={{ display: "grid", gap: 24 }}>
          <div style={{ display: "grid", gap: 24, gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
            ...
            <div style={{ background: C.slate, border: `1px solid ${C.line}`, borderRadius: 14, padding: 24 }}>
              ...
              <Field label="Your utility">
                <div style={{ display: "flex", gap: 8 }}>
                  {UTILITIES.map((u) => (
                    <Toggle key={u.id} active={utilityId === u.id} onClick={() => pickUtility(u.id)}>{u.name}</Toggle>
                  ))}
                </div>
                <div className="mono" style={{ fontSize: 11.5, color: C.dim, marginTop: 8 }}>
                  {utility.region} · est. ${rate.toFixed(2)}/kWh · {sunHrs} sun hrs/day
                </div>
              </Field>
              ...
              <Field label="How do you want to enter usage?">
                ...
                <div style={{ marginTop: 12, padding: "10px 12px", background: C.night, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12.5, color: C.dim, lineHeight: 1.6 }}>
                  ...
                  {utility.link ? (
                    ...
                    <>
                      <a href="https://myaccount.sdge.com" target="_blank" rel="noopener noreferrer" style={{ color: C.gold, textDecoration: "underline" }}>SDG&E</a>
                      {" · "}
                      <a href="https://m.pge.com" target="_blank" rel="noopener noreferrer" style={{ color: C.gold, textDecoration: "underline" }}>PG&E</a>
                    </>
                  )}
              ...
              <Field label={`Rate: $${rate.toFixed(2)}/kWh`} hint="Fine-tune if you know the actual average rate">
                <Slider value={rate} min={0.08} max={0.60} step={0.01} onChange={setRate} />
              </Field>
      ...
      <div className="print-report">
        ...
        <div style={{ fontSize: 13, lineHeight: 1.7 }}>
          ...
          <div>
            Utility: <b>{utility.name}</b> at ~${rate.toFixed(2)}/kWh · Solar resource: <b>{sunHrs} peak sun hrs/day</b><br />
